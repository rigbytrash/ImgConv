...
int main(int argc, char **argv){
    ...
    image->err = readInFile(image, "ebf", argv[1]); // read in the data + perform checks; 0 idicates success
    if (image->err != 0){
        return image->err;
    }

    image->err = printEBF(image, argv[2]); // prints the data in the specified format
    if (image->err != 0){
        return image->err;
    }
