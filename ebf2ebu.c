...
int main(int argc, char **argv)
    ...
    switch(checkData(image, inputFilename)){
        case 0:
    ...
    // now we have finished using the image->associatedFile we should close it
    fclose(image->associatedFile);

    // open the output file in write mode
    image->associatedFile = fopen(outputFilename, "w");
    
    switch(printEBU(image, outputFilename, check)){ // file perms validation happens here
        case 0:
            free(image->imageData);
            return BAD_OUTPUT;
            break;
