...
int main(int argc, char **argv){
    ...
    image1->err = readInFile(image1, "ebu", argv[1]); // read in the data + perform checks; 0 idicates success
    if (image1->err != 0){
        return image1->err;
    }

    Image *image2 = newImage(); // create struct to store data
    image2->err = readInFile(image2, "ebu", argv[2]); // read in the data + perform checks; 0 idicates success
    if (image2->err != 0){
        return image2->err;
    }
